...
def mock_read():
    ...
    with patch("moe.plugins.read.read_item", autospec=True) as mock_read:
        yield mock_read
...
def mock_query() -> Iterator[FunctionType]:
    ...
    Yields:
        Mock query
    """
    with patch("moe.plugins.read.read_cli.cli_query", autospec=True) as mock_query:
        yield mock_query
...
class TestCommand:
    ...
    def test_rm_item(self, mock_query, mock_read):
        ...
        with patch("moe.plugins.read.read_cli.remove.remove_item") as mock_rm:
            moe.cli.main(cli_args)
