...
class TestConfiguration:
    ...
    def test_remove_plugin_exists(self, tmp_config):
        """The 'add cli' plugin relies on the 'remove' plugin so it must exist."""
        tmp_config(settings="default_plugins = ['add', 'remove']")


@pytest.fixture
def tmp_add_config(tmp_config) -> Config:
    """A temporary config for the add plugin with the cli."""
    return tmp_config('default_plugins = ["cli", "add", "remove"]')
...
class TestPreAdd:
    ...
    def test_merge_track(self, tmp_session, mock_track, tmp_add_config):
        ...
        with patch(
            "moe.plugins.add.add_cli.choice_prompt", autospec=True
    ...
    def test_replace_album(self, mock_album_factory, tmp_session, tmp_add_config):
        ...
        with patch(
            "moe.plugins.add.add_cli.choice_prompt",
            return_value=mock_prompt_choice,
            autospec=True,
        ):
            tmp_add_config.plugin_manager.hook.pre_add(
...
class TestPluginRegistration:
    ...
    def test_cli_no_remove(self, caplog, tmp_config):
        """Log a warning if the remove plugin is not enabled.

        The 'add cli' plugin depends on the remove plugin for duplicate resolution.
        """
        tmp_config(settings='default_plugins = ["add", "cli"]')

        assert any(record.levelname == "WARNING" for record in caplog.records)

    def test_cli_remove(self, caplog, tmp_config):
        """Don't log a warning if the remove plugin is enabled."""
        tmp_config(settings='default_plugins = ["add", "cli", "remove"]')

        assert not any(record.levelname == "WARNING" for record in caplog.records)

    def test_no_cli_no_remove(self, caplog, tmp_config):
        """Only require the remove plugin for the cli."""
