...
class TestCollectionCommand:
    ...
    def test_track(self, mock_query):
        ...
        with patch(
            "moe.plugins.musicbrainz.mb_cli.moe_mb.set_collection",
            autospec=True,
        ) as mock_set:
            moe.cli.main(cli_args)

        mock_set.assert_called_once_with({"123"})
        mock_query.assert_called_once_with(ANY, "*", "track")
    ...
    def test_extra(self, mock_query):
        ...
        with patch(
            "moe.plugins.musicbrainz.mb_cli.moe.plugins.musicbrainz.set_collection",
            autospec=True,
        ) as mock_set:
            moe.cli.main(cli_args)

        mock_set.assert_called_once_with({"123"})
        mock_query.assert_called_once_with(ANY, "*", "extra")
    ...
    def test_remove(self, mock_query):
        ...
        with patch(
            "moe.plugins.musicbrainz.mb_cli.moe.plugins.musicbrainz.rm_releases_from_collection",
            autospec=True,
        ) as mock_rm:
            moe.cli.main(cli_args)
    ...
    def test_add(self, mock_query):
        ...
        with patch(
            "moe.plugins.musicbrainz.mb_cli.moe.plugins.musicbrainz.add_releases_to_collection",
            autospec=True,
        ) as mock_add:
            moe.cli.main(cli_args)
...
class TestAddImportPromptChoice:
    ...
    def test_enter_id(self, tmp_config):
        ...
        with (
            ...
            patch(
                "moe.plugins.musicbrainz.mb_cli.moe.plugins.musicbrainz.get_candidate_by_id",
