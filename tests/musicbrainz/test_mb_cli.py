...
def mock_query() -> Iterator[FunctionType]:
    ...
    Yields:
        Mock query
    """
    with patch("moe.musicbrainz.mb_cli.cli_query", autospec=True) as mock_query:
        yield mock_query
...
class TestCollectionCommand:
    ...
    def test_track(self, mock_query):
        ...
        with patch(
            "moe.musicbrainz.mb_cli.moe_mb.set_collection", autospec=True
        ) as mock_set:
    ...
    def test_remove(self, mock_query):
        ...
        with patch(
            "moe.musicbrainz.mb_cli.moe_mb.rm_releases_from_collection",
            autospec=True,
        ) as mock_rm:
            moe.cli.main(cli_args)
    ...
    def test_add(self, mock_query):
        ...
        with patch(
            "moe.musicbrainz.mb_cli.moe_mb.add_releases_to_collection",
            autospec=True,
        ) as mock_add:
            moe.cli.main(cli_args)
...
class TestAddImportPromptChoice:
    ...
    def test_enter_id(self, tmp_config):
        ...
        mock_album = Mock()
        with patch(
            "moe.musicbrainz.mb_cli.questionary.text",
            **{"return_value.ask.return_value": "new id"}
        ):
            with patch(
                "moe.musicbrainz.mb_cli.moe_mb.get_candidate_by_id",
                return_value=mock_album,
                autospec=True,
            ) as mock_get_candidate:
                with patch(
                    "moe.musicbrainz.mb_cli.moe_import.import_prompt",
                    autospec=True,
                ) as mock_prompt:
