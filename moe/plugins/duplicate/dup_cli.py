...
def resolve_dup_items(config: Config, item_a: LibItem, item_b: LibItem):
    ...
    # func(config, item_a, item_b) # noqa: E800
    prompt_choices = [
    ...
    prompt_choice.func(config, item_a, item_b)


def _keep_a(config: Config, item_a: LibItem, item_b: LibItem):
...
def _keep_b(config: Config, item_a: LibItem, item_b: LibItem):
    """Keeps `item_a`, removing `item_b` from the library."""
...
def _merge(config: Config, item_a: LibItem, item_b: LibItem):
...
def _overwrite(config: Config, item_a: LibItem, item_b: LibItem):
